...
static const char* get_mods_name(int mods) {
  ...
  if (mods & GLFW_MOD_CAPS_LOCK) strcat(name, " capslock-on");
  if (mods & GLFW_MOD_NUM_LOCK) strcat(name, " numlock-on");
...
static void framebuffer_size_callback(GLFWwindow* window, int width,
  ...
  printf("%08x to %i at %0.3f: Framebuffer size: %i %i\n", counter++,
         slot->number, glfwGetTime(), width, height);
}

static void window_content_scale_callback(GLFWwindow* window, float xscale,
                                          float yscale) {
  Slot* slot = glfwGetWindowUserPointer(window);
  printf("%08x to %i at %0.3f: Window content scale: %0.3f %0.3f\n", counter++,
         slot->number, glfwGetTime(), xscale, yscale);
}
...
static void mouse_button_callback(GLFWwindow* window, int button, int action,
  ...
  printf("%08x to %i at %0.3f: Mouse button %i (%s) (with%s) was %s\n",
         ...
         get_button_name(button), get_mods_name(mods), get_action_name(action));
}
...
static void cursor_enter_callback(GLFWwindow* window, int entered) {
  ...
  printf("%08x to %i at %0.3f: Cursor %s window\n", counter++, slot->number,
         glfwGetTime(), entered ? "entered" : "left");
}
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action,
  ...
  if (name) {
    printf(
        "%08x to %i at %0.3f: Key 0x%04x Scancode 0x%04x (%s) (%s) (with%s) "
        "was %s\n",
        counter++, slot->number, glfwGetTime(), key, scancode,
        get_key_name(key), name, get_mods_name(mods), get_action_name(action));
  } else {
    printf(
        "%08x to %i at %0.3f: Key 0x%04x Scancode 0x%04x (%s) (with%s) was "
        "%s\n",
        counter++, slot->number, glfwGetTime(), key, scancode,
        get_key_name(key), get_mods_name(mods), get_action_name(action));
  }
...
static void char_callback(GLFWwindow* window, unsigned int codepoint) {
  ...
  printf("%08x to %i at %0.3f: Character 0x%08x (%s) input\n", counter++,
         slot->number, glfwGetTime(), codepoint, string);
}
...
static void monitor_callback(GLFWmonitor* monitor, int event) {
  if (event == GLFW_CONNECTED) {
    ...
    printf(
        ...
        counter++, glfwGetTime(), glfwGetMonitorName(monitor), mode->width,
        mode->height, x, y, widthMM, heightMM);
  } else if (event == GLFW_DISCONNECTED) {
    printf("%08x at %0.3f: Monitor %s was disconnected\n", counter++,
           glfwGetTime(), glfwGetMonitorName(monitor));
  }
}
...
static void joystick_callback(int jid, int event) {
  if (event == GLFW_CONNECTED) {
    ...
    printf(
        "%08x at %0.3f: Joystick %i (%s) was connected with %i axes, %i "
        "buttons, and %i hats\n",
        counter++, glfwGetTime(), jid, glfwGetJoystickName(jid), axisCount,
...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (monitor) {
      printf("Creating full screen window %i (%ix%i on %s)\n", slots[i].number,
             width, height, glfwGetMonitorName(monitor));
    } else {
    ...
    glfwSetWindowContentScaleCallback(slots[i].window,
  ...
  for (;;) {
    for (i = 0; i < count; i++) {
      if (glfwWindowShouldClose(slots[i].window)) break;
    }
