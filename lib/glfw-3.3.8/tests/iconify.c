...
static int windowed_xpos, windowed_ypos, windowed_width = 640,
                                         windowed_height = 480;
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action,
  ...
  switch (key) {
    ...
    case GLFW_KEY_ESCAPE:
      ...
      break;
    case GLFW_KEY_A:
      glfwSetWindowAttrib(window, GLFW_AUTO_ICONIFY,
                          !glfwGetWindowAttrib(window, GLFW_AUTO_ICONIFY));
      break;
    case GLFW_KEY_B:
...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < window_count; i++) {
    ...
    printf("Window is %s and %s\n",
           glfwGetWindowAttrib(windows[i], GLFW_ICONIFIED) ? "iconified"
                                                           : "restored",
           glfwGetWindowAttrib(windows[i], GLFW_FOCUSED) ? "focused"
                                                         : "defocused");
  }
  ...
  for (;;) {
    ...
    for (i = 0; i < window_count; i++) {
      if (glfwWindowShouldClose(windows[i])) break;
    }
