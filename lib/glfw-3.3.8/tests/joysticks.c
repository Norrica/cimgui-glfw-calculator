...
static void joystick_callback(int jid, int event) {
  if (event == GLFW_CONNECTED)
    ...
    for (i = 0; i < joystick_count; i++) {
      if (joysticks[i] == jid) break;
    }

    for (i = i + 1; i < joystick_count; i++) joysticks[i - 1] = joysticks[i];
...
static void drop_callback(GLFWwindow* window, int count, const char* paths[]) {
  ...
  for (i = 0; i < count; i++) {
    ...
    text = malloc(size + 1);
    text[size] = '\0';
    if (fread(text, 1, size, stream) == size) glfwUpdateGamepadMappings(text);
...
static void hat_widget(struct nk_context* nk, unsigned char state) {
  ...
  if (nk_widget(&area, nk) == NK_WIDGET_INVALID) return;
  ...
  nk_stroke_circle(
      ...
      nk_rect(center.x - radius, center.y - radius, radius * 2.f, radius * 2.f),
      1.f, nk_rgb(175, 175, 175));

  if (state) {
    const float angles[] = {
        0.f, 0.f,           NK_PI * 1.5f,  NK_PI * 1.75f, NK_PI,
        0.f, NK_PI * 1.25f, 0.f,           NK_PI * 0.5f,  NK_PI * 0.25f,
        0.f, 0.f,           NK_PI * 0.75f, 0.f,
    };
    ...
    nk_fill_triangle(
        nk_window_get_canvas(nk), center.x + cosa * p0.x + sina * p0.y,
...
int main(void) {
  ...
  for (jid = GLFW_JOYSTICK_1; jid <= GLFW_JOYSTICK_LAST; jid++) {
    if (glfwJoystickPresent(jid)) joysticks[joystick_count++] = jid;
  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (i = 0; i < joystick_count; i++) {
      if (nk_begin(nk, joystick_label(joysticks[i]),
        ...
        if (!hat_buttons) button_count -= hat_count * 4;
        ...
        for (j = 0; j < hat_count; j++) hat_widget(nk, hats[j]);
        ...
        if (glfwGetGamepadState(joysticks[i], &state)) {
          ...
          nk_labelf(nk, NK_TEXT_LEFT, "Gamepad state: %s",
          ...
          if (state.buttons[GLFW_GAMEPAD_BUTTON_DPAD_UP]) hat |= GLFW_HAT_UP;
