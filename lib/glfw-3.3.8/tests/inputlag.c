...
void update_vsync() { glfwSwapInterval(enable_vsync == nk_true ? 1 : 0); }
...
void swap_buffers(GLFWwindow* window) {
  ...
  if (swap_clear) glClear(GL_COLOR_BUFFER_BIT);

  if (swap_finish) glFinish();

  if (swap_occlusion_query) {
    GLint occlusion_result;
    if (!occlusion_query) glGenQueries(1, &occlusion_query);
...
void draw_marker(struct nk_command_buffer* canvas, int lead,
                 struct nk_vec2 pos) {
  struct nk_color colors[4] = {nk_rgb(255, 0, 0), nk_rgb(255, 255, 0),
                               nk_rgb(0, 255, 0), nk_rgb(0, 96, 255)};
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (nk_begin(nk, "", area, 0)) {
      ...
      if (nk_option_label(nk, "glfwGetCursorPos (sync query)",
                          cursor_method == cursor_sync_query))
        cursor_method = cursor_sync_query;
      ...
      if (nk_checkbox_label(nk, "Enable vsync", &enable_vsync)) update_vsync();
