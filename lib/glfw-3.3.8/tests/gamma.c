...
static void chart_ramp_array(struct nk_context* nk, struct nk_color color,
                             int count, unsigned short int* values) {
  if (nk_chart_begin_colored(nk, NK_CHART_LINES, color, nk_rgb(255, 255, 255),
                             count, 0, 65535)) {
    int i;
    for (i = 0; i < count; i++) {
      char buffer[1024];
      if (nk_chart_push(nk, values[i])) {
        snprintf(buffer, sizeof(buffer), "#%u: %u (%0.5f) ", i, values[i],
                 values[i] / 65535.f);
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (nk_begin(nk, "", area, 0)) {
      ...
      if (nk_button_label(nk, "Revert")) glfwSetGammaRamp(monitor, &orig_ramp);
