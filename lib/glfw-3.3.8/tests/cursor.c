...
static void key_callback(GLFWwindow* window, int key, int scancode, int action,
  ...
  switch (key) {
    case GLFW_KEY_A: {
      ...
      if (!animate_cursor) glfwSetCursor(window, NULL);
    ...
    case GLFW_KEY_R:
      if (!glfwRawMouseMotionSupported()) break;
...
int main(void) {
  ...
  glVertexAttribPointer(vpos_location, 2, GL_FLOAT, GL_FALSE, sizeof(vec2),
                        (void*)0);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (track_cursor) {
      ...
      vertices[0][0] = 0.5f;
      vertices[0][1] =
          (float)(fb_height - floor(cursor_y * scale) - 1.f + 0.5f);
  ...
  for (i = 0; i < CURSOR_FRAME_COUNT; i++) glfwDestroyCursor(star_cursors[i]);
