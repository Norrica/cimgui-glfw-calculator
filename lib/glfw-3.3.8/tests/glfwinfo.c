...
static void usage(void) {
  ...
  printf("Options:\n");
  printf("  -a, --client-api=API      the client API to use (" API_NAME_OPENGL
         " or " API_NAME_OPENGL_ES ")\n");
  printf(
      "  -b, --behavior=BEHAVIOR   the release behavior to use "
      "(" BEHAVIOR_NAME_NONE " or " BEHAVIOR_NAME_FLUSH ")\n");
  printf(
      "  -c, --context-api=API     the context creation API to use "
      "(" API_NAME_NATIVE " or " API_NAME_EGL " or " API_NAME_OSMESA ")\n");
  printf("  -d, --debug               request a debug context\n");
  printf("  -f, --forward             require a forward-compatible context\n");
  printf("  -h, --help                show this help\n");
  printf(
      "  -l, --list-extensions     list all Vulkan and client API "
      "extensions\n");
  printf("      --list-layers         list all Vulkan layers\n");
  printf(
      "  -m, --major=MAJOR         the major number of the required "
      "client API version\n");
  printf(
      "  -n, --minor=MINOR         the minor number of the required "
      "client API version\n");
  printf(
      "  -p, --profile=PROFILE     the OpenGL profile to use "
      "(" PROFILE_NAME_CORE " or " PROFILE_NAME_COMPAT ")\n");
  printf(
      "  -s, --robustness=STRATEGY the robustness strategy to use "
      "(" STRATEGY_NAME_NONE " or " STRATEGY_NAME_LOSE ")\n");
  ...
  printf(
      "      --no-error            request a context that does not emit "
      "errors\n");
...
static const char* get_profile_name_gl(GLint mask) {
  if (mask & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) return PROFILE_NAME_COMPAT;
  if (mask & GL_CONTEXT_CORE_PROFILE_BIT) return PROFILE_NAME_CORE;
...
static void list_vulkan_instance_layers(void) {
  ...
  if (vkEnumerateInstanceLayerProperties(&lp_count, NULL) != VK_SUCCESS) return;
  ...
  for (i = 0; i < lp_count; i++) {
    printf(" %s (v%u) \"%s\"\n", lp[i].layerName, lp[i].specVersion >> 22,
           lp[i].description);
  }
...
int main(int argc, char** argv) {
  ...
  if (!valid_version()) exit(EXIT_FAILURE);
  ...
  while ((ch = getopt_long(argc, argv, "a:b:c:dfhlm:n:p:s:v", options, NULL)) !=
         -1) {
    switch (ch) {
      case 'a':
      case CLIENT:
  ...
  if (client == GLFW_OPENGL_API) {
    if (major >= 3) {
      ...
      if (flags & 2 /*GL_CONTEXT_FLAG_DEBUG_BIT*/) printf(" debug");
      if (flags & GL_CONTEXT_FLAG_ROBUST_ACCESS_BIT_ARB) printf(" robustness");
      if (flags & 8 /*GL_CONTEXT_FLAG_NO_ERROR_BIT_KHR*/) printf(" no-error");
      putchar('\n');
      ...
      if (glfwGetWindowAttrib(window, GLFW_OPENGL_DEBUG_CONTEXT))
        printf(" debug");
      if (glfwGetWindowAttrib(window, GLFW_CONTEXT_ROBUSTNESS) ==
    ...
    if (GLAD_GL_ARB_robustness) {
      const int robustness =
          glfwGetWindowAttrib(window, GLFW_CONTEXT_ROBUSTNESS);
      GLint strategy;
      glGetIntegerv(GL_RESET_NOTIFICATION_STRATEGY_ARB, &strategy);

      printf("%s robustness strategy (0x%08x): %s\n", get_api_name(client),
             strategy, get_strategy_name_gl(strategy));

      printf("%s robustness strategy parsed by GLFW: %s\n",
             get_api_name(client), get_strategy_name_glfw(robustness));
    }
  }
  ...
  if (major >= 2) {
    printf("%s context shading language version: \"%s\"\n",
           get_api_name(client), glGetString(GL_SHADING_LANGUAGE_VERSION));
  }
  ...
  if (client == GLFW_OPENGL_ES_API || GLAD_GL_ARB_multisample || major > 1 ||
      minor >= 3) {
  ...
  if (client == GLFW_OPENGL_API && profile != GLFW_OPENGL_CORE_PROFILE) {
    ...
    printf(
        " accum red: %u accum green: %u accum blue: %u accum alpha: %u aux "
        "buffers: %u\n",
        accumredbits, accumgreenbits, accumbluebits, accumalphabits,
        auxbuffers);
  }

  if (list_extensions) list_context_extensions(client, major, minor);
  ...
  if (glfwVulkanSupported()) {
    ...
    if (list_layers) list_vulkan_instance_layers();

    ai.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    ai.pApplicationName = "glfwinfo";
    ai.applicationVersion = VK_MAKE_VERSION(
    ...
    for (i = 0; i < pd_count; i++) {
      ...
      printf("Vulkan %s device: \"%s\" API version %i.%i\n",
             get_device_type_name(pdp.deviceType), pdp.deviceName,
      ...
      VkExtensionProperties* ep =
          calloc(ep_count, sizeof(VkExtensionProperties));
      ...
      if (list_layers) list_vulkan_device_layers(instance, pd[i]);
    }
