...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (i) glfwWindowHint(GLFW_FOCUS_ON_SHOW, focusOnShow);
    ...
    glfwSetWindowPos(windows[i], 100 + (i & 1) * (200 + left + right),
                     100 + (i >> 1) * (200 + top + bottom));
  }

  for (i = 0; i < 4; i++) glfwShowWindow(windows[i]);
  ...
  while (running) {
    for (i = 0; i < 4; i++) {
      ...
      if (glfwWindowShouldClose(windows[i])) running = GLFW_FALSE;
    }
