...
void cursor_position_callback(GLFWwindow* window, double x, double y) {
  ...
  if (override_pos) set_ball_pos(cursor_x, cursor_y);
}
...
void DrawBoingBallBand(GLfloat long_lo, GLfloat long_hi) {
  ...
  for (lat_deg = 0; lat_deg <= (360 - STEP_LATITUDE);
     ...
     */
    if (drawBallHow == DRAW_BALL_SHADOW) glColor3f(0.35f, 0.35f, 0.35f);
  ...
  /*
   * Toggle color so that next band will opposite red/white colors than this
   * one.
   */
  colorToggle = !colorToggle;
