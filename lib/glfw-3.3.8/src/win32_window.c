...
static const GLFWimage* chooseImage(int count, const GLFWimage* images,
  ...
  for (i = 0; i < count; i++) {
    const int currDiff =
        abs(images[i].width * images[i].height - width * height);
...
//
static HICON createIcon(const GLFWimage* image, int xhot, int yhot,
                        GLFWbool icon) {
  int i;
  HDC dc;
  HICON handle;
...
//
static void getFullWindowSize(DWORD style, DWORD exStyle, int contentWidth,
                              int contentHeight, int* fullWidth,
...
static void applyAspectRatio(_GLFWwindow* window, int edge, RECT* area) {
  ...
  if (edge == WMSZ_LEFT || edge == WMSZ_BOTTOMLEFT || edge == WMSZ_RIGHT ||
    ...
    area->bottom =
        area->top + yoff + (int)((area->right - area->left - xoff) / ratio);
  } else if (edge == WMSZ_TOPLEFT || edge == WMSZ_TOPRIGHT) {
    area->top =
        area->bottom - yoff - (int)((area->right - area->left - xoff) / ratio);
  } else if (edge == WMSZ_TOP || edge == WMSZ_BOTTOM) {
    area->right =
        area->left + xoff + (int)((area->bottom - area->top - yoff) * ratio);
  }
}
...
static void disableCursor(_GLFWwindow* window) {
  ...
  _glfwPlatformGetCursorPos(window, &_glfw.win32.restoreCursorPosX,
...
static void acquireMonitor(_GLFWwindow* window) {
  if (!_glfw.win32.acquiredMonitorCount) {
    ...
    if (IsWindowsXPOrGreater()) {
      SystemParametersInfoW(SPI_GETMOUSETRAILS, 0, &_glfw.win32.mouseTrailSize,
                            0);
...
static void releaseMonitor(_GLFWwindow* window) {
  ...
  if (!_glfw.win32.acquiredMonitorCount) {
    ...
    if (IsWindowsXPOrGreater())
      SystemParametersInfoW(SPI_SETMOUSETRAILS, _glfw.win32.mouseTrailSize, 0,
                            0);
  }
...
static void maximizeWindowManually(_GLFWwindow* window) {
  ...
  GetMonitorInfoW(
      MonitorFromWindow(window->win32.handle, MONITOR_DEFAULTTONEAREST), &mi);
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
  ...
  switch (uMsg) {
          ...
          if (PeekMessageW(&next, NULL, 0, 0, PM_NOREMOVE)) {
            if (next.message == WM_KEYDOWN || next.message == WM_SYSKEYDOWN ||
              ...
              if (next.wParam == VK_MENU &&
                  (HIWORD(next.lParam) & KF_EXTENDED) && next.time == time) {
    ...
    case WM_MOUSEHWHEEL: {
      ...
      _glfwInputScroll(window, -((SHORT)HIWORD(wParam) / (double)WHEEL_DELTA),
                       0.0);
      return 0;
    }
    ...
    case WM_SIZE: {
      ...
      if (_glfw.win32.disabledCursorWindow == window) updateClipRect(window);
    ...
    case WM_GETDPISCALEDSIZE: {
      ...
      if (_glfwIsWindows10CreatorsUpdateOrGreaterWin32()) {
        ...
        AdjustWindowRectExForDpi(&source, getWindowStyle(window), FALSE,
                                 getWindowExStyle(window),
                                 GetDpiForWindow(window->win32.handle));
        AdjustWindowRectExForDpi(&target, getWindowStyle(window), FALSE,
    ...
    case WM_DPICHANGED: {
      ...
      if (!window->monitor &&
        ...
        SetWindowPos(window->win32.handle, HWND_TOP, suggested->left,
...
static int createNativeWindow(_GLFWwindow* window,
  ...
  if (window->monitor) {
    ...
    getFullWindowSize(style, exStyle, wndconfig->width, wndconfig->height,
                      &fullWidth, &fullHeight, USER_DEFAULT_SCREEN_DPI);
  }
  ...
  window->win32.handle =
      CreateWindowExW(exStyle, _GLFW_WNDCLASSNAME, wideTitle, style, xpos, ypos,
  ...
  if (IsWindows7OrGreater()) {
    ChangeWindowMessageFilterEx(window->win32.handle, WM_DROPFILES,
                                MSGFLT_ALLOW, NULL);
    ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYDATA, MSGFLT_ALLOW,
                                NULL);
  ...
  if (!window->monitor) {
    ...
    OffsetRect(&rect, wp.rcNormalPosition.left - rect.left,
    ...
    // Adjust rect of maximized undecorated window, because by default Windows
    // will make such a window cover the whole monitor instead of its workarea

    if (wndconfig->maximized && !wndconfig->decorated) {
      MONITORINFO mi = {sizeof(mi)};
      GetMonitorInfoW(mh, &mi);

      SetWindowPos(window->win32.handle, HWND_TOP, mi.rcWork.left,
                   mi.rcWork.top, mi.rcWork.right - mi.rcWork.left,
  ...
  _glfwPlatformGetWindowSize(window, &window->win32.width,
                             &window->win32.height);
...
GLFWbool _glfwRegisterWindowClassWin32(void) {
  ...
  if (!wc.hIcon) {
    ...
    wc.hIcon = LoadImageW(NULL, IDI_APPLICATION, IMAGE_ICON, 0, 0,
                          LR_DEFAULTSIZE | LR_SHARED);
  }
...
void _glfwPlatformSetWindowIcon(_GLFWwindow* window, int count,
  ...
  if (count) {
    const GLFWimage* bigImage =
        chooseImage(count, images, GetSystemMetrics(SM_CXICON),
                    GetSystemMetrics(SM_CYICON));
    const GLFWimage* smallImage =
        chooseImage(count, images, GetSystemMetrics(SM_CXSMICON),
...
void _glfwPlatformSetWindowPos(_GLFWwindow* window, int xpos, int ypos) {
  ...
  if (_glfwIsWindows10AnniversaryUpdateOrGreaterWin32()) {
    AdjustWindowRectExForDpi(&rect, getWindowStyle(window), FALSE,
                             getWindowExStyle(window),
                             GetDpiForWindow(window->win32.handle));
  } else {
    AdjustWindowRectEx(&rect, getWindowStyle(window), FALSE,
                       getWindowExStyle(window));
  }
...
void _glfwPlatformSetWindowSize(_GLFWwindow* window, int width, int height) {
  if (window->monitor) {
    ...
    SetWindowPos(
        window->win32.handle, HWND_TOP, 0, 0, rect.right - rect.left,
...
void _glfwPlatformWaitEventsTimeout(double timeout) {
  MsgWaitForMultipleObjects(0, NULL, FALSE, (DWORD)(timeout * 1e3),
                            QS_ALLEVENTS);
...
int _glfwPlatformGetKeyScancode(int key) { return _glfw.win32.scancodes[key]; }
...
int _glfwPlatformCreateStandardCursor(_GLFWcursor* cursor, int shape) {
  ...
  cursor->win32.handle = LoadImageW(NULL, MAKEINTRESOURCEW(id), IMAGE_CURSOR, 0,
