...
static GLFWbool waitForAnyEvent(double* timeout) {
  nfds_t count = 2;
  struct pollfd fds[3] = {{ConnectionNumber(_glfw.x11.display), POLLIN},
...
static void drainEmptyEvents(void) {
  for (;;) {
    ...
    const ssize_t result =
        read(_glfw.x11.emptyEventPipe[0], dummy, sizeof(dummy));
...
static GLFWbool waitForVisibilityNotify(_GLFWwindow* window) {
  ...
  while (!XCheckTypedWindowEvent(_glfw.x11.display, window->x11.handle,
...
static int getWindowState(_GLFWwindow* window) {
  ...
  if (_glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.WM_STATE,
                                _glfw.x11.WM_STATE,
...
//
static Bool isSelectionEvent(Display* display, XEvent* event,
                             XPointer pointer) {
...
//
static Bool isFrameExtentsEvent(Display* display, XEvent* event,
                                XPointer pointer) {
...
//
static Bool isSelPropNewValueNotify(Display* display, XEvent* event,
                                    XPointer pointer) {
...
//
static void sendEventToWM(_GLFWwindow* window, Atom type, long a, long b,
...
static void updateWindowMode(_GLFWwindow* window) {
  if (window->monitor) {
    if (_glfw.x11.xinerama.available && _glfw.x11.NET_WM_FULLSCREEN_MONITORS) {
      sendEventToWM(window, _glfw.x11.NET_WM_FULLSCREEN_MONITORS,
                    window->monitor->x11.index, window->monitor->x11.index,
                    window->monitor->x11.index, window->monitor->x11.index, 0);
    }
    ...
    if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_FULLSCREEN) {
      ...
      XChangeWindowAttributes(_glfw.x11.display, window->x11.handle,
...
static void disableCursor(_GLFWwindow* window) {
  ...
  _glfwPlatformGetCursorPos(window, &_glfw.x11.restoreCursorPosX,
                            &_glfw.x11.restoreCursorPosY);
  updateCursorImage(window);
  _glfwCenterCursorInContentArea(window);
  XGrabPointer(_glfw.x11.display, window->x11.handle, True,
               ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
               GrabModeAsync, GrabModeAsync, window->x11.handle,
               _glfw.x11.hiddenCursorHandle, CurrentTime);
}
...
static void enableCursor(_GLFWwindow* window) {
  ...
  _glfwPlatformSetCursorPos(window, _glfw.x11.restoreCursorPosX,
                            _glfw.x11.restoreCursorPosY);
  updateCursorImage(window);
}
...
static GLFWbool createNativeWindow(_GLFWwindow* window,
  ...
  window->x11.colormap =
      XCreateColormap(_glfw.x11.display, _glfw.x11.root, visual, AllocNone);
  ...
  window->x11.handle = XCreateWindow(
      _glfw.x11.display, _glfw.x11.root, 0, 0,  // Position
      width, height,
      0,      // Border width
      depth,  // Color depth
  ...
  XSaveContext(_glfw.x11.display, window->x11.handle, _glfw.x11.context,
               (XPointer)window);
  ...
  // Declare our PID
  {
    ...
    XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_PID,
  ...
  // Announce support for Xdnd (drag and drop)
  {
    ...
    XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.XdndAware,
  ...
  if (_glfw.x11.im) {
    window->x11.ic =
        XCreateIC(_glfw.x11.im, XNInputStyle,
  ...
  if (window->x11.ic) {
    ...
    if (XGetICValues(window->x11.ic, XNFilterEvents, &filter, NULL) == NULL)
      XSelectInput(_glfw.x11.display, window->x11.handle,
                   wa.event_mask | filter);
  }
...
static Atom writeTargetToProperty(const XSelectionRequestEvent* request) {
  ...
  if (request->target == _glfw.x11.TARGETS) {
    ...
    const Atom targets[] = {_glfw.x11.TARGETS, _glfw.x11.MULTIPLE,
                            _glfw.x11.UTF8_STRING, XA_STRING};

    XChangeProperty(_glfw.x11.display, request->requestor, request->property,
  ...
  if (request->target == _glfw.x11.MULTIPLE) {
    ...
    count = _glfwGetWindowPropertyX11(request->requestor, request->property,
                                      _glfw.x11.ATOM_PAIR,
    ...
    for (i = 0; i < count; i += 2) {
      ...
      if (j < formatCount) {
        XChangeProperty(_glfw.x11.display, request->requestor, targets[i + 1],
                        targets[i], 8, PropModeReplace,
  ...
  if (request->target == _glfw.x11.SAVE_TARGETS) {
    ...
    XChangeProperty(_glfw.x11.display, request->requestor, request->property,
                    _glfw.x11.NULL_, 32, PropModeReplace, NULL, 0);
...
static const char* getSelectionString(Atom selection) {
  ...
  *selectionString = NULL;
  ...
  for (size_t i = 0; i < targetCount; i++) {
    ...
    XConvertSelection(_glfw.x11.display, selection, targets[i],
                      _glfw.x11.GLFW_SELECTION, _glfw.x11.helperWindowHandle,
                      CurrentTime);
    ...
    XCheckIfEvent(_glfw.x11.display, &dummy, isSelPropNewValueNotify,
                  (XPointer)&notification);

    XGetWindowProperty(_glfw.x11.display, notification.xselection.requestor,
                       notification.xselection.property, 0, LONG_MAX, True,
    ...
    if (actualType == _glfw.x11.INCR) {
      ...
      for (;;) {
        while (!XCheckIfEvent(_glfw.x11.display, &dummy,
...
static void acquireMonitor(_GLFWwindow* window) {
  if (_glfw.x11.saver.count == 0) {
    ...
    XGetScreenSaver(_glfw.x11.display, &_glfw.x11.saver.timeout,
                    &_glfw.x11.saver.interval, &_glfw.x11.saver.blanking,
  ...
  if (window->x11.overrideRedirect) {
    ...
    XMoveResizeWindow(_glfw.x11.display, window->x11.handle, xpos, ypos,
...
static void releaseMonitor(_GLFWwindow* window) {
  ...
  if (_glfw.x11.saver.count == 0) {
    ...
    XSetScreenSaver(_glfw.x11.display, _glfw.x11.saver.timeout,
                    _glfw.x11.saver.interval, _glfw.x11.saver.blanking,
                    _glfw.x11.saver.exposure);
  }
}
...
static void processEvent(XEvent* event) {
  ...
  if (XFindContext(_glfw.x11.display, event->xany.window, _glfw.x11.context,
                   (XPointer*)&window) != 0) {
  ...
  switch (event->type) {
    ...
    case KeyPress: {
      ...
      if (window->x11.ic) {
        ...
        if (!filtered) {
          ...
          count = Xutf8LookupString(window->x11.ic, &event->xkey, buffer,
                                    sizeof(buffer) - 1, NULL, &status);

          if (status == XBufferOverflow) {
            chars = calloc(count + 1, 1);
            count = Xutf8LookupString(window->x11.ic, &event->xkey, chars,
                                      count, NULL, &status);
          }
          ...
          count =
              XwcLookupString(window->x11.ic, &event->xkey, buffer,
                              sizeof(buffer) / sizeof(wchar_t), NULL, &status);

          if (status == XBufferOverflow) {
            chars = calloc(count, sizeof(wchar_t));
            count = XwcLookupString(window->x11.ic, &event->xkey, chars, count,
                                    NULL, &status);
          }
    ...
    case ButtonPress: {
      ...
      // Modern X provides scroll events as mouse button presses
        ...
        _glfwInputMouseClick(window, event->xbutton.button - Button1 - 4,
                             GLFW_PRESS, mods);
      }
    ...
    case ConfigureNotify: {
      if (event->xconfigure.width != window->x11.width ||
        ...
        _glfwInputFramebufferSize(window, event->xconfigure.width,
                                  event->xconfigure.height);

        _glfwInputWindowSize(window, event->xconfigure.width,
                             event->xconfigure.height);
      ...
      if (!event->xany.send_event && window->x11.parent != _glfw.x11.root) {
        ...
        Window dummy;
        XTranslateCoordinates(_glfw.x11.display, window->x11.parent,
                              _glfw.x11.root, xpos, ypos, &xpos, &ypos, &dummy);
      ...
      if (event->xclient.message_type == _glfw.x11.WM_PROTOCOLS) {
        ...
        if (list) {
          count = _glfwGetWindowPropertyX11(_glfw.x11.xdnd.source,
                                            _glfw.x11.XdndTypeList, XA_ATOM,
        ...
        if (_glfw.x11.xdnd.format) {
          ...
          XConvertSelection(_glfw.x11.display, _glfw.x11.XdndSelection,
                            _glfw.x11.xdnd.format, _glfw.x11.XdndSelection,
          ...
          XSendEvent(_glfw.x11.display, _glfw.x11.xdnd.source, False,
                     NoEventMask, &reply);
        ...
        XTranslateCoordinates(_glfw.x11.display, _glfw.x11.root,
                              window->x11.handle, xabs, yabs, &xpos, &ypos,
                              &dummy);
        ...
        XSendEvent(_glfw.x11.display, _glfw.x11.xdnd.source, False, NoEventMask,
                   &reply);
        XFlush(_glfw.x11.display);
      }
    ...
    case SelectionNotify: {
      if (event->xselection.property == _glfw.x11.XdndSelection) {
        ...
        char* data;
        const unsigned long result = _glfwGetWindowPropertyX11(
...
unsigned long _glfwGetWindowPropertyX11(Window window, Atom property, Atom type,
  ...
  XGetWindowProperty(_glfw.x11.display, window, property, 0, LONG_MAX, False,
...
void _glfwPushSelectionToManagerX11(void) {
  XConvertSelection(_glfw.x11.display, _glfw.x11.CLIPBOARD_MANAGER,
...
#if defined(X_HAVE_UTF8_STRING)
  Xutf8SetWMProperties(_glfw.x11.display, window->x11.handle, title, title,
                       NULL, 0, NULL, NULL, NULL);
#else
  // This may be a slightly better fallback than using XStoreName and
  // XSetIconName, which always store their arguments using STRING
  XmbSetWMProperties(_glfw.x11.display, window->x11.handle, title, title, NULL,
                     0, NULL, NULL, NULL);
#endif

  XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_NAME,
                  _glfw.x11.UTF8_STRING, 8, PropModeReplace,
...
void _glfwPlatformSetWindowIcon(_GLFWwindow* window, int count,
  ...
  if (count) {
    ...
    // NOTE: XChangeProperty expects 32-bit values like the image data above to
    // be
    //       placed in the 32 least significant bits of individual longs.  This
    //       is true even if long is 64-bit and a WM protocol calls for "packed"
    //       data. This is because of a historical mistake that then became part
    //       of the Xlib ABI.  Xlib will pack these values into a regular array
    //       of 32-bit values before sending it over the wire.
    XChangeProperty(_glfw.x11.display, window->x11.handle,
                    _glfw.x11.NET_WM_ICON, XA_CARDINAL, 32, PropModeReplace,
...
void _glfwPlatformSetWindowPos(_GLFWwindow* window, int xpos, int ypos) {
  ...
  if (!_glfwPlatformWindowVisible(window)) {
    ...
    if (XGetWMNormalHints(_glfw.x11.display, window->x11.handle, hints,
                          &supplied)) {
...
void _glfwPlatformGetWindowFrameSize(_GLFWwindow* window, int* left, int* top,
  ...
  if (!_glfwPlatformWindowVisible(window) &&
    ...
    while (!XCheckIfEvent(_glfw.x11.display, &event, isFrameExtentsEvent,
                          (XPointer)window)) {
  ...
  if (_glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.NET_FRAME_EXTENTS,
                                XA_CARDINAL, (unsigned char**)&extents) == 4) {
...
void _glfwPlatformIconifyWindow(_GLFWwindow* window) {
  if (window->x11.overrideRedirect) {
    ...
    _glfwInputError(GLFW_PLATFORM_ERROR,
                    "X11: Iconification of full screen windows requires a WM "
                    "that supports EWMH full screen");
    return;
  }
  ...
  if (_glfwPlatformWindowIconified(window)) {
    ...
    if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_MAXIMIZED_VERT &&
      ...
      sendEventToWM(window, _glfw.x11.NET_WM_STATE, _NET_WM_STATE_REMOVE,
                    ...
                    _glfw.x11.NET_WM_STATE_MAXIMIZED_HORZ, 1, 0);
    }
  }
...
void _glfwPlatformMaximizeWindow(_GLFWwindow* window) {
  ...
  if (_glfwPlatformWindowVisible(window)) {
    ...
    Atom* states = NULL;
    unsigned long count =
        _glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.NET_WM_STATE,
...
void _glfwPlatformShowWindow(_GLFWwindow* window) {
  if (_glfwPlatformWindowVisible(window)) return;
...
void _glfwPlatformFocusWindow(_GLFWwindow* window) {
  if (_glfw.x11.NET_ACTIVE_WINDOW)
    ...
    XSetInputFocus(_glfw.x11.display, window->x11.handle, RevertToParent,
                   CurrentTime);
  }
...
void _glfwPlatformSetWindowMonitor(_GLFWwindow* window, _GLFWmonitor* monitor,
  ...
  if (window->monitor) {
    ...
    updateWindowMode(window);
    XMoveResizeWindow(_glfw.x11.display, window->x11.handle, xpos, ypos, width,
                      height);
  }
...
int _glfwPlatformWindowHovered(_GLFWwindow* window) {
  ...
  while (w) {
    ...
    const Bool result = XQueryPointer(_glfw.x11.display, w, &root, &w, &rootX,
...
void _glfwPlatformSetWindowFloating(_GLFWwindow* window, GLFWbool enabled) {
  ...
  if (_glfwPlatformWindowVisible(window)) {
    ...
    sendEventToWM(window, _glfw.x11.NET_WM_STATE, action,
                  _glfw.x11.NET_WM_STATE_ABOVE, 0, 1, 0);
  } else {
    Atom* states = NULL;
    unsigned long i, count;
...
void _glfwPlatformPostEmptyEvent(void) { writeEmptyEvent(); }
...
void _glfwPlatformSetCursorMode(_GLFWwindow* window, int mode) {
  if (mode == GLFW_CURSOR_DISABLED) {
    if (_glfwPlatformWindowFocused(window)) disableCursor(window);
...
int _glfwPlatformGetKeyScancode(int key) { return _glfw.x11.scancodes[key]; }
