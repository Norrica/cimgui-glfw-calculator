...
GLFWAPI void glfwGetMonitorPhysicalSize(GLFWmonitor* handle, int* widthMM,
                                        int* heightMM) {
  _GLFWmonitor* monitor = (_GLFWmonitor*)handle;
  assert(monitor != NULL);
...
GLFWAPI void glfwGetMonitorContentScale(GLFWmonitor* handle, float* xscale,
                                        float* yscale) {
  _GLFWmonitor* monitor = (_GLFWmonitor*)handle;
  assert(monitor != NULL);
...
GLFWAPI const GLFWgammaramp* glfwGetGammaRamp(GLFWmonitor* handle) {
  ...
  _glfwFreeGammaArrays(&monitor->currentRamp);
  if (!_glfwPlatformGetGammaRamp(monitor, &monitor->currentRamp)) return NULL;
...
GLFWAPI void glfwSetGammaRamp(GLFWmonitor* handle, const GLFWgammaramp* ramp) {
  ...
  if (ramp->size <= 0) {
    _glfwInputError(GLFW_INVALID_VALUE, "Invalid gamma ramp size %i",
                    ramp->size);
    return;
  }

  if (!monitor->originalRamp.size) {
    if (!_glfwPlatformGetGammaRamp(monitor, &monitor->originalRamp)) return;
  }
