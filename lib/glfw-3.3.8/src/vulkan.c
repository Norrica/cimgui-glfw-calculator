...
GLFWbool _glfwInitVulkan(int mode) {
  ...
  if (!_glfw.vk.handle) _glfw.vk.handle = _glfwLoadLocalVulkanLoaderNS();
  ...
  _glfw.vk.GetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)_glfw_dlsym(
...
#if !defined(_GLFW_VULKAN_STATIC)
  if (_glfw.vk.handle) _glfw_dlclose(_glfw.vk.handle);
#endif
}
...
#else
  if (!proc) proc = (GLFWvkproc)_glfw_dlsym(_glfw.vk.handle, procname);
#endif
...
GLFWAPI int glfwGetPhysicalDevicePresentationSupport(VkInstance instance,
  ...
  return _glfwPlatformGetPhysicalDevicePresentationSupport(instance, device,
                                                           queuefamily);
}
  ...
  *surface = VK_NULL_HANDLE;
  ...
  if (window->context.client != GLFW_NO_API) {
    _glfwInputError(GLFW_INVALID_VALUE,
                    "Vulkan: Window surface creation requires the window to "
                    "have the client API set to GLFW_NO_API");
