...
#pragma message( \
    "These symbols must be exported by the executable and have no effect in a DLL")
#endif
...
static GLFWbool loadLibraries(void) {
  ...
  _glfw.win32.user32.SetProcessDPIAware_ =
      (PFN_SetProcessDPIAware)GetProcAddress(_glfw.win32.user32.instance,
                                             "SetProcessDPIAware");
  _glfw.win32.user32.ChangeWindowMessageFilterEx_ =
      (PFN_ChangeWindowMessageFilterEx)GetProcAddress(
          _glfw.win32.user32.instance, "ChangeWindowMessageFilterEx");
  _glfw.win32.user32.EnableNonClientDpiScaling_ =
      (PFN_EnableNonClientDpiScaling)GetProcAddress(
          _glfw.win32.user32.instance, "EnableNonClientDpiScaling");
  ...
  _glfw.win32.user32.GetDpiForWindow_ = (PFN_GetDpiForWindow)GetProcAddress(
  ...
  _glfw.win32.user32.GetSystemMetricsForDpi_ =
      (PFN_GetSystemMetricsForDpi)GetProcAddress(_glfw.win32.user32.instance,
                                                 "GetSystemMetricsForDpi");

  _glfw.win32.dinput8.instance = LoadLibraryA("dinput8.dll");
  if (_glfw.win32.dinput8.instance) {
    _glfw.win32.dinput8.Create = (PFN_DirectInput8Create)GetProcAddress(
      ...
      if (_glfw.win32.xinput.instance) {
        _glfw.win32.xinput.GetCapabilities =
            (PFN_XInputGetCapabilities)GetProcAddress(
                _glfw.win32.xinput.instance, "XInputGetCapabilities");
        _glfw.win32.xinput.GetState = (PFN_XInputGetState)GetProcAddress(
  ...
  if (_glfw.win32.dwmapi.instance) {
    _glfw.win32.dwmapi.IsCompositionEnabled =
        (PFN_DwmIsCompositionEnabled)GetProcAddress(_glfw.win32.dwmapi.instance,
                                                    "DwmIsCompositionEnabled");
    _glfw.win32.dwmapi.Flush =
        (PFN_DwmFlush)GetProcAddress(_glfw.win32.dwmapi.instance, "DwmFlush");
    _glfw.win32.dwmapi.EnableBlurBehindWindow =
        (PFN_DwmEnableBlurBehindWindow)GetProcAddress(
            _glfw.win32.dwmapi.instance, "DwmEnableBlurBehindWindow");
    _glfw.win32.dwmapi.GetColorizationColor =
        (PFN_DwmGetColorizationColor)GetProcAddress(_glfw.win32.dwmapi.instance,
  ...
  if (_glfw.win32.shcore.instance) {
    ...
    _glfw.win32.shcore.GetDpiForMonitor_ = (PFN_GetDpiForMonitor)GetProcAddress(
  ...
  if (_glfw.win32.ntdll.instance) {
    _glfw.win32.ntdll.RtlVerifyVersionInfo_ =
        (PFN_RtlVerifyVersionInfo)GetProcAddress(_glfw.win32.ntdll.instance,
...
static void freeLibraries(void) {
  if (_glfw.win32.xinput.instance) FreeLibrary(_glfw.win32.xinput.instance);

  if (_glfw.win32.dinput8.instance) FreeLibrary(_glfw.win32.dinput8.instance);
...
static GLFWbool createHelperWindow(void) {
  ...
  _glfw.win32.helperWindowHandle =
      CreateWindowExW(WS_EX_OVERLAPPEDWINDOW, _GLFW_WNDCLASSNAME,
                      L"GLFW message window", WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
                      0, 0, 1, 1, NULL, NULL, _glfw.win32.instance, NULL);
  ...
  // Register for HID device notifications
  {
    ...
    _glfw.win32.deviceNotificationHandle = RegisterDeviceNotificationW(
        _glfw.win32.helperWindowHandle, (DEV_BROADCAST_HDR*)&dbi,
...
void _glfwInputErrorWin32(int error, const char* description) {
  ...
  FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS |
                 ...
                 NULL, GetLastError() & 0xffff,
                 MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), buffer,
                 sizeof(buffer) / sizeof(WCHAR), NULL);
  WideCharToMultiByte(CP_UTF8, 0, buffer, -1, message, sizeof(message), NULL,
                      NULL);
...
void _glfwUpdateKeyNamesWin32(void) {
  ...
  for (key = GLFW_KEY_SPACE; key <= GLFW_KEY_LAST; key++) {
    ...
    if (key >= GLFW_KEY_KP_0 && key <= GLFW_KEY_KP_ADD) {
      const UINT vks[] = {VK_NUMPAD0,  VK_NUMPAD1,  VK_NUMPAD2, VK_NUMPAD3,
    ...
    length =
        ToUnicode(vk, scancode, state, chars, sizeof(chars) / sizeof(WCHAR), 0);

    if (length == -1) {
      // This is a dead key, so we need a second simulated key press
      // to make it output its own character (usually a diacritic)
      length = ToUnicode(vk, scancode, state, chars,
                         sizeof(chars) / sizeof(WCHAR), 0);
    }
...
int _glfwPlatformInit(void) {
  ...
  if (!_glfwRegisterWindowClassWin32()) return GLFW_FALSE;
