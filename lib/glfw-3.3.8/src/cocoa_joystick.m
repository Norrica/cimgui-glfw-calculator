...
static CFComparisonResult compareElements(const void* fp, const void* sp, void* user) {
  ...
  if (fe->usage < se->usage) return kCFCompareLessThan;
  if (fe->usage > se->usage) return kCFCompareGreaterThan;
  if (fe->index < se->index) return kCFCompareLessThan;
  if (fe->index > se->index) return kCFCompareGreaterThan;
...
static void matchCallback(void* context, IOReturn result, void* sender, IOHIDDeviceRef device) {
  ...
  for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++) {
    if (_glfw.joysticks[jid].ns.device == device) return;
  }
  ...
  if (property) CFNumberGetValue(property, kCFNumberSInt32Type, &vendor);

  property = IOHIDDeviceGetProperty(device, CFSTR(kIOHIDProductIDKey));
  if (property) CFNumberGetValue(property, kCFNumberSInt32Type, &product);

  property = IOHIDDeviceGetProperty(device, CFSTR(kIOHIDVersionNumberKey));
  if (property) CFNumberGetValue(property, kCFNumberSInt32Type, &version);

  // Generate a joystick GUID that matches the SDL 2.0.5+ one
  if (vendor && product) {
    sprintf(guid, "03000000%02x%02x0000%02x%02x0000%02x%02x0000", (uint8_t)vendor,
            (uint8_t)(vendor >> 8), (uint8_t)product, (uint8_t)(product >> 8), (uint8_t)version,
            (uint8_t)(version >> 8));
  } else {
...
void _glfwInitJoysticksNS(void) {
  ...
  const long usages[] = {kHIDUsage_GD_Joystick, kHIDUsage_GD_GamePad,
...
void _glfwTerminateJoysticksNS(void) {
  ...
  for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++) {
    if (_glfw.joysticks[jid].connected) closeJoystick(&_glfw.joysticks[jid]);
  }
