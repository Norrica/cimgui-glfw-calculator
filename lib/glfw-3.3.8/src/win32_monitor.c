...
//
static BOOL CALLBACK monitorCallback(HMONITOR handle, HDC dc, RECT* rect,
                                     LPARAM data) {
  MONITORINFOEXW mi;
...
static _GLFWmonitor* createMonitor(DISPLAY_DEVICEW* adapter,
  ...
  WideCharToMultiByte(CP_UTF8, 0, adapter->DeviceName, -1,
                      monitor->win32.publicAdapterName,
                      sizeof(monitor->win32.publicAdapterName), NULL, NULL);

  if (display) {
    wcscpy(monitor->win32.displayName, display->DeviceName);
    WideCharToMultiByte(CP_UTF8, 0, display->DeviceName, -1,
...
void _glfwPollMonitorsWin32(void) {
  ...
  if (disconnectedCount) {
    ...
    memcpy(disconnected, _glfw.monitors,
  ...
  for (adapterIndex = 0;; adapterIndex++) {
    ...
    for (displayIndex = 0;; displayIndex++) {
      ...
      for (i = 0; i < disconnectedCount; i++) {
        if (disconnected[i] && wcscmp(disconnected[i]->win32.displayName,
                                      display.DeviceName) == 0) {
          disconnected[i] = NULL;
          // handle may have changed, update
          EnumDisplayMonitors(NULL, NULL, monitorCallback,
                              (LPARAM)_glfw.monitors[i]);
          break;
        }
      }
    ...
    if (displayIndex == 0) {
      for (i = 0; i < disconnectedCount; i++) {
        if (disconnected[i] && wcscmp(disconnected[i]->win32.adapterName,
...
void _glfwSetVideoModeWin32(_GLFWmonitor* monitor, const GLFWvidmode* desired) {
  ...
  result = ChangeDisplaySettingsExW(monitor->win32.adapterName, &dm, NULL,
                                    CDS_FULLSCREEN, NULL);
  if (result == DISP_CHANGE_SUCCESSFUL)
    ...
    _glfwInputError(GLFW_PLATFORM_ERROR, "Win32: Failed to set video mode: %s",
                    description);
  }
}
...
void _glfwRestoreVideoModeWin32(_GLFWmonitor* monitor) {
  if (monitor->win32.modeChanged) {
    ChangeDisplaySettingsExW(monitor->win32.adapterName, NULL, NULL,
                             CDS_FULLSCREEN, NULL);
    monitor->win32.modeChanged = GLFW_FALSE;
  }
}

void _glfwGetMonitorContentScaleWin32(HMONITOR handle, float* xscale,
                                      float* yscale) {
  UINT xdpi, ydpi;
  ...
  if (IsWindows8Point1OrGreater()) {
    if (GetDpiForMonitor(handle, MDT_EFFECTIVE_DPI, &xdpi, &ydpi) != S_OK) {
      _glfwInputError(GLFW_PLATFORM_ERROR,
                      "Win32: Failed to query monitor DPI");
      return;
    }
  } else {
...
void _glfwPlatformGetMonitorPos(_GLFWmonitor* monitor, int* xpos, int* ypos) {
  ...
  EnumDisplaySettingsExW(monitor->win32.adapterName, ENUM_CURRENT_SETTINGS, &dm,
                         EDS_ROTATEDMODE);
...
void _glfwPlatformGetMonitorContentScale(_GLFWmonitor* monitor, float* xscale,
                                         float* yscale) {
  _glfwGetMonitorContentScaleWin32(monitor->win32.handle, xscale, yscale);
}

void _glfwPlatformGetMonitorWorkarea(_GLFWmonitor* monitor, int* xpos,
...
GLFWvidmode* _glfwPlatformGetVideoModes(_GLFWmonitor* monitor, int* count) {
  ...
  *count = 0;
  ...
  for (;;) {
    ...
    _glfwSplitBPP(dm.dmBitsPerPel, &mode.redBits, &mode.greenBits,
                  &mode.blueBits);
    ...
    if (monitor->win32.modesPruned) {
      ...
      if (ChangeDisplaySettingsExW(monitor->win32.adapterName, &dm, NULL,
