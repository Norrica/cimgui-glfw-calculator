...
//
static int findPixelFormatAttribValue(const int* attribs, int attribCount,
...
static int choosePixelFormat(_GLFWwindow* window,
  ...
  if (_glfw.wgl.ARB_pixel_format) {
    ...
    if (!wglGetPixelFormatAttribivARB(window->context.wgl.dc, 1, 0, 1, &attrib,
                                      &nativeCount)) {
      _glfwInputErrorWin32(GLFW_PLATFORM_ERROR,
    ...
    if (_glfw.wgl.ARB_multisample) addAttrib(WGL_SAMPLES_ARB);

    if (ctxconfig->client == GLFW_OPENGL_API) {
      if (_glfw.wgl.ARB_framebuffer_sRGB || _glfw.wgl.EXT_framebuffer_sRGB)
        addAttrib(WGL_FRAMEBUFFER_SRGB_CAPABLE_ARB);
    } else {
      if (_glfw.wgl.EXT_colorspace) addAttrib(WGL_COLORSPACE_EXT);
    }
  } else {
    nativeCount = DescribePixelFormat(window->context.wgl.dc, 1,
  ...
  for (i = 0; i < nativeCount; i++) {
    ...
    if (_glfw.wgl.ARB_pixel_format) {
      ...
      if (!wglGetPixelFormatAttribivARB(window->context.wgl.dc, pixelFormat, 0,
                                        attribCount, attribs, values)) {
        _glfwInputErrorWin32(GLFW_PLATFORM_ERROR,
      ...
      if (findAttribValue(WGL_PIXEL_TYPE_ARB) != WGL_TYPE_RGBA_ARB) continue;
      ...
      if (findAttribValue(WGL_STEREO_ARB)) u->stereo = GLFW_TRUE;
      ...
      if (!DescribePixelFormat(window->context.wgl.dc, pixelFormat,
...
GLFWbool _glfwInitWGL(void) {
  ...
  _glfw.wgl.CreateContext = (PFN_wglCreateContext)GetProcAddress(
      _glfw.wgl.instance, "wglCreateContext");
  _glfw.wgl.DeleteContext = (PFN_wglDeleteContext)GetProcAddress(
      _glfw.wgl.instance, "wglDeleteContext");
  _glfw.wgl.GetProcAddress = (PFN_wglGetProcAddress)GetProcAddress(
      _glfw.wgl.instance, "wglGetProcAddress");
  _glfw.wgl.GetCurrentDC = (PFN_wglGetCurrentDC)GetProcAddress(
      _glfw.wgl.instance, "wglGetCurrentDC");
  _glfw.wgl.GetCurrentContext = (PFN_wglGetCurrentContext)GetProcAddress(
      _glfw.wgl.instance, "wglGetCurrentContext");
  _glfw.wgl.MakeCurrent =
      (PFN_wglMakeCurrent)GetProcAddress(_glfw.wgl.instance, "wglMakeCurrent");
  _glfw.wgl.ShareLists =
      (PFN_wglShareLists)GetProcAddress(_glfw.wgl.instance, "wglShareLists");
...
void _glfwTerminateWGL(void) {
  if (_glfw.wgl.instance) FreeLibrary(_glfw.wgl.instance);
}
...
GLFWbool _glfwCreateContextWGL(_GLFWwindow* window,
  ...
  if (!DescribePixelFormat(window->context.wgl.dc, pixelFormat, sizeof(pfd),
                           &pfd)) {
  ...
  if (_glfw.wgl.ARB_create_context) {
    ...
    if (flags) setAttrib(WGL_CONTEXT_FLAGS_ARB, flags);

    if (mask) setAttrib(WGL_CONTEXT_PROFILE_MASK_ARB, mask);
      ...
      if (error == (0xc0070000 | ERROR_INVALID_VERSION_ARB)) {
        ...
        _glfwInputError(
            ...
            "WGL: Driver does not support the requested OpenGL profile");
                 ...
                 (0xc0070000 | ERROR_INCOMPATIBLE_DEVICE_CONTEXTS_ARB)) {
        _glfwInputError(GLFW_INVALID_VALUE,
                        "WGL: The share context is not compatible with the "
                        "requested context");
      } else {
        if (ctxconfig->client == GLFW_OPENGL_API) {
          _glfwInputError(GLFW_VERSION_UNAVAILABLE,
