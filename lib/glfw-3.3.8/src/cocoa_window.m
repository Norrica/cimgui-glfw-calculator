...
static void updateCursorMode(_GLFWwindow* window) {
  if (window->cursorMode == GLFW_CURSOR_DISABLED) {
    ...
    _glfwPlatformGetCursorPos(window, &_glfw.ns.restoreCursorPosX, &_glfw.ns.restoreCursorPosY);
    _glfwCenterCursorInContentArea(window);
    CGAssociateMouseAndMouseCursorPosition(false);
  } else if (_glfw.ns.disabledCursorWindow == window) {
    _glfw.ns.disabledCursorWindow = NULL;
    _glfwPlatformSetCursorPos(window, _glfw.ns.restoreCursorPosX, _glfw.ns.restoreCursorPosY);
...
static void acquireMonitor(_GLFWwindow* window) {
  ...
  const NSRect frame =
      NSMakeRect(bounds.origin.x, _glfwTransformYNS(bounds.origin.y + bounds.size.height - 1),
...
static int translateFlags(NSUInteger flags) {
  ...
  if (flags & NSEventModifierFlagCapsLock) mods |= GLFW_MOD_CAPS_LOCK;
...
static int translateKey(unsigned int key) {
  if (key >= sizeof(_glfw.ns.keycodes) / sizeof(_glfw.ns.keycodes[0])) return GLFW_KEY_UNKNOWN;
...
- (void)windowDidResize:(NSNotification*)notification {
  if (window->context.source == GLFW_NATIVE_CONTEXT_API) [window->context.nsgl.object update];

  if (_glfw.ns.disabledCursorWindow == window) _glfwCenterCursorInContentArea(window);
...
- (void)otherMouseDown:(NSEvent*)event {
  _glfwInputMouseClick(window, (int)[event buttonNumber], GLFW_PRESS,
                       translateFlags([event modifierFlags]));
}
...
- (void)otherMouseUp:(NSEvent*)event {
  _glfwInputMouseClick(window, (int)[event buttonNumber], GLFW_RELEASE,
                       translateFlags([event modifierFlags]));
}

- (void)mouseExited:(NSEvent*)event {
  if (window->cursorMode == GLFW_CURSOR_HIDDEN) showCursor(window);
...
- (void)scrollWheel:(NSEvent*)event {
  ...
  if (fabs(deltaX) > 0.0 || fabs(deltaY) > 0.0) _glfwInputScroll(window, deltaX, deltaY);
}
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  if (count) {
    ...
    for (NSUInteger i = 0; i < count; i++) free(paths[i]);
    free(paths);
  }
...
static GLFWbool createNativeWindow(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
  ...
  window->ns.object = [[GLFWWindow alloc] initWithContentRect:contentRect
                                                    styleMask:styleMask
  ...
  if (window->monitor)
    ...
    _glfw.ns.cascadePoint = NSPointToCGPoint(
        [window->ns.object cascadeTopLeftFromPoint:NSPointFromCGPoint(_glfw.ns.cascadePoint)]);
    ...
    if (wndconfig->floating) [window->ns.object setLevel:NSFloatingWindowLevel];

    if (wndconfig->maximized) [window->ns.object zoom:nil];
  }
...
//
float _glfwTransformYNS(float y) { return CGDisplayBounds(CGMainDisplayID()).size.height - y - 1; }
...
int _glfwPlatformCreateWindow(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
  ...
  @autoreleasepool {
    if (!_glfw.ns.finishedLaunching) [NSApp run];
    ...
    if (ctxconfig->client != GLFW_NO_API) {
      if (ctxconfig->source == GLFW_NATIVE_CONTEXT_API) {
        ...
        if (!_glfwCreateContextNSGL(window, ctxconfig, fbconfig)) return GLFW_FALSE;
...
void _glfwPlatformDestroyWindow(_GLFWwindow* window) {
  @autoreleasepool {
    if (_glfw.ns.disabledCursorWindow == window) _glfw.ns.disabledCursorWindow = NULL;
...
void _glfwPlatformGetWindowPos(_GLFWwindow* window, int* xpos, int* ypos) {
  @autoreleasepool {
    ...
    if (xpos) *xpos = contentRect.origin.x;
    if (ypos) *ypos = _glfwTransformYNS(contentRect.origin.y + contentRect.size.height - 1);
...
void _glfwPlatformSetWindowPos(_GLFWwindow* window, int x, int y) {
  @autoreleasepool {
    ...
    const NSRect dummyRect =
        NSMakeRect(x, _glfwTransformYNS(y + contentRect.size.height - 1), 0, 0);
...
void _glfwPlatformGetWindowSize(_GLFWwindow* window, int* width, int* height) {
  @autoreleasepool {
    ...
    if (width) *width = contentRect.size.width;
    if (height) *height = contentRect.size.height;
...
void _glfwPlatformSetWindowSizeLimits(_GLFWwindow* window, int minwidth, int minheight,
...
void _glfwPlatformGetFramebufferSize(_GLFWwindow* window, int* width, int* height) {
  @autoreleasepool {
    ...
    if (width) *width = (int)fbRect.size.width;
    if (height) *height = (int)fbRect.size.height;

  }  // autoreleasepool
}

void _glfwPlatformGetWindowFrameSize(_GLFWwindow* window, int* left, int* top, int* right,
                                     int* bottom) {
  @autoreleasepool {
    const NSRect contentRect = [window->ns.view frame];
    const NSRect frameRect = [window->ns.object frameRectForContentRect:contentRect];

    if (left) *left = contentRect.origin.x - frameRect.origin.x;
    if (top)
      *top = frameRect.origin.y + frameRect.size.height - contentRect.origin.y -
             contentRect.size.height;
    if (right)
      *right =
          frameRect.origin.x + frameRect.size.width - contentRect.origin.x - contentRect.size.width;
    if (bottom) *bottom = contentRect.origin.y - frameRect.origin.y;
...
void _glfwPlatformMaximizeWindow(_GLFWwindow* window) {
  @autoreleasepool {
    if (![window->ns.object isZoomed]) [window->ns.object zoom:nil];
  }  // autoreleasepool
}
...
void _glfwPlatformSetWindowMonitor(_GLFWwindow* window, _GLFWmonitor* monitor, int xpos, int ypos,
  ...
  @autoreleasepool {
    ...
    if (window->monitor) {
      ...
      NSRect contentRect = NSMakeRect(xpos, _glfwTransformYNS(ypos + height - 1), width, height);
...
int _glfwPlatformWindowHovered(_GLFWwindow* window) {
  @autoreleasepool {
    ...
    if ([NSWindow windowNumberAtPoint:point
            belowWindowWithWindowNumber:0] != [window->ns.object windowNumber]) {
      return GLFW_FALSE;
    }

    return NSMouseInRect(point, [window->ns.object convertRectToScreen:[window->ns.view frame]],
                         NO);
...
void _glfwPlatformGetCursorPos(_GLFWwindow* window, double* xpos, double* ypos) {
  @autoreleasepool {
    ...
    if (xpos) *xpos = pos.x;
    if (ypos) *ypos = contentRect.size.height - pos.y;
...
void _glfwPlatformSetCursorPos(_GLFWwindow* window, double x, double y) {
  @autoreleasepool {
    ...
    if (window->monitor) {
      ...
      CGWarpMouseCursorPosition(CGPointMake(globalPoint.x, _glfwTransformYNS(globalPoint.y)));
    }
    ...
    if (window->cursorMode != GLFW_CURSOR_DISABLED) CGAssociateMouseAndMouseCursorPosition(true);
...
void _glfwPlatformSetCursorMode(_GLFWwindow* window, int mode) {
  @autoreleasepool {
    if (_glfwPlatformWindowFocused(window)) updateCursorMode(window);
  }  // autoreleasepool
}

const char* _glfwPlatformGetScancodeName(int scancode) {
  @autoreleasepool {
    if (scancode < 0 || scancode > 0xff || _glfw.ns.keycodes[scancode] == GLFW_KEY_UNKNOWN) {
    ...
    if (UCKeyTranslate([(NSData*)_glfw.ns.unicodeData bytes], scancode, kUCKeyActionDisplay, 0,
                       LMGetKbdType(), kUCKeyTranslateNoDeadKeysBit, &deadKeyState,
                       sizeof(characters) / sizeof(characters[0]), &characterCount,
                       characters) != noErr) {
      return NULL;
    }
    ...
    CFStringRef string = CFStringCreateWithCharactersNoCopy(kCFAllocatorDefault, characters,
                                                            characterCount, kCFAllocatorNull);
    CFStringGetCString(string, _glfw.ns.keynames[key], sizeof(_glfw.ns.keynames[key]),
...
int _glfwPlatformGetKeyScancode(int key) { return _glfw.ns.scancodes[key]; }
...
int _glfwPlatformCreateCursor(_GLFWcursor* cursor, const GLFWimage* image, int xhot, int yhot) {
  @autoreleasepool {
    ...
    if (cursor->ns.object == nil) return GLFW_FALSE;
...
int _glfwPlatformCreateStandardCursor(_GLFWcursor* cursor, int shape) {
  @autoreleasepool {
    ...
    if (!cursor->ns.object) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "Cocoa: Failed to retrieve standard cursor");
      return GLFW_FALSE;
    }
...
void _glfwPlatformDestroyCursor(_GLFWcursor* cursor) {
  @autoreleasepool {
    if (cursor->ns.object) [(NSCursor*)cursor->ns.object release];
  }  // autoreleasepool
}
...
const char* _glfwPlatformGetClipboardString(void) {
  @autoreleasepool {
    ...
    if (![[pasteboard types] containsObject:NSPasteboardTypeString]) {
      _glfwInputError(GLFW_FORMAT_UNAVAILABLE, "Cocoa: Failed to retrieve string from pasteboard");
      return NULL;
    }
...
int _glfwPlatformGetPhysicalDevicePresentationSupport(VkInstance instance, VkPhysicalDevice device,
                                                      uint32_t queuefamily) {
  return GLFW_TRUE;
}

VkResult _glfwPlatformCreateWindowSurface(VkInstance instance, _GLFWwindow* window,
  ...
  @autoreleasepool {
    ...
    if (!bundle) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "Cocoa: Failed to find QuartzCore.framework");
    ...
    if (!window->ns.layer) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "Cocoa: Failed to create layer for view");
      return VK_ERROR_EXTENSION_NOT_PRESENT;
    }
    ...
    if (err) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "Cocoa: Failed to create Vulkan surface: %s",
