...
void _glfwInputJoystick(_GLFWjoystick* js, int event) {
  ...
  if (_glfw.callbacks.joystick) _glfw.callbacks.joystick(jid, event);
}
...
GLFWAPI void glfwSetInputMode(GLFWwindow* handle, int mode, int value) {
  ...
  if (mode == GLFW_CURSOR) {
    ...
    _glfwPlatformGetCursorPos(window, &window->virtualCursorPosX,
...
GLFWAPI void glfwSetCursorPos(GLFWwindow* handle, double xpos, double ypos) {
  ...
  if (xpos != xpos || xpos < -DBL_MAX || xpos > DBL_MAX || ypos != ypos ||
      ypos < -DBL_MAX || ypos > DBL_MAX) {
    _glfwInputError(GLFW_INVALID_VALUE, "Invalid cursor position %f %f", xpos,
                    ypos);
    return;
  }

  if (!_glfwPlatformWindowFocused(window)) return;
...
GLFWAPI GLFWcursor* glfwCreateStandardCursor(int shape) {
  ...
  if (shape != GLFW_ARROW_CURSOR && shape != GLFW_IBEAM_CURSOR &&
      shape != GLFW_CROSSHAIR_CURSOR && shape != GLFW_HAND_CURSOR &&
      shape != GLFW_HRESIZE_CURSOR && shape != GLFW_VRESIZE_CURSOR) {
...
GLFWAPI GLFWcharmodsfun glfwSetCharModsCallback(GLFWwindow* handle,
...
glfwSetCursorEnterCallback(GLFWwindow* handle, GLFWcursorenterfun cbfun) {
  _GLFWwindow* window = (_GLFWwindow*)handle;
  assert(window != NULL);
...
GLFWAPI const float* glfwGetJoystickAxes(int jid, int* count) {
  ...
  *count = 0;
  ...
  if (!_glfwPlatformPollJoystick(js, _GLFW_POLL_AXES)) return NULL;
...
GLFWAPI const unsigned char* glfwGetJoystickButtons(int jid, int* count) {
  ...
  *count = 0;
  ...
  if (!_glfwPlatformPollJoystick(js, _GLFW_POLL_BUTTONS)) return NULL;
...
GLFWAPI const char* glfwGetJoystickName(int jid) {
  ...
  if (!_glfwPlatformPollJoystick(js, _GLFW_POLL_PRESENCE)) return NULL;
        ...
        if (parseMapping(&mapping, line)) {
          ...
          if (previous)
            *previous = mapping;
            ...
            _glfw.mappingCount++;
            _glfw.mappings = realloc(_glfw.mappings,
