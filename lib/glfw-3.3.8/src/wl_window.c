...
static struct wl_buffer* createShmBuffer(const GLFWimage* image) {
  ...
  if (data == MAP_FAILED) {
    _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Failed to map file: %s",
                    strerror(errno));
    close(fd);
    return NULL;
  }
  ...
  buffer = wl_shm_pool_create_buffer(pool, 0, image->width, image->height,
...
static void createFallbackDecoration(_GLFWdecorationWayland* decoration,
  ...
  decoration->subsurface = wl_subcompositor_get_subsurface(
      _glfw.wl.subcompositor, decoration->surface, parent);
  wl_subsurface_set_position(decoration->subsurface, x, y);
  decoration->viewport =
      wp_viewporter_get_viewport(_glfw.wl.viewporter, decoration->surface);
...
static void createFallbackDecorations(_GLFWwindow* window) {
  ...
  if (!window->wl.decorations.buffer) return;

  createFallbackDecoration(&window->wl.decorations.top, window->wl.surface,
                           window->wl.decorations.buffer, 0,
                           -GLFW_CAPTION_HEIGHT, window->wl.width,
                           GLFW_CAPTION_HEIGHT);
  createFallbackDecoration(&window->wl.decorations.left, window->wl.surface,
                           window->wl.decorations.buffer, -GLFW_BORDER_SIZE,
                           -GLFW_CAPTION_HEIGHT, GLFW_BORDER_SIZE,
                           window->wl.height + GLFW_CAPTION_HEIGHT);
  createFallbackDecoration(&window->wl.decorations.right, window->wl.surface,
                           window->wl.decorations.buffer, window->wl.width,
                           -GLFW_CAPTION_HEIGHT, GLFW_BORDER_SIZE,
                           window->wl.height + GLFW_CAPTION_HEIGHT);
  createFallbackDecoration(
      &window->wl.decorations.bottom, window->wl.surface,
      window->wl.decorations.buffer, -GLFW_BORDER_SIZE, window->wl.height,
...
static void xdgDecorationHandleConfigure(
    void* userData, struct zxdg_toplevel_decoration_v1* decoration,
    uint32_t mode) {
...
static void resizeWindow(_GLFWwindow* window) {
  ...
  if (window->wl.egl.window)
    wl_egl_window_resize(window->wl.egl.window, scaledWidth, scaledHeight, 0,
                         0);
  if (!window->wl.transparent) setContentAreaOpaque(window);
  _glfwInputFramebufferSize(window, scaledWidth, scaledHeight);

  if (!window->wl.decorations.top.surface) return;
...
static void surfaceHandleEnter(void* userData, struct wl_surface* surface,
  ...
  if (window->wl.monitorsCount + 1 > window->wl.monitorsSize) {
    ...
    window->wl.monitors = realloc(
        window->wl.monitors, window->wl.monitorsSize * sizeof(_GLFWmonitor*));
  }
...
static void surfaceHandleLeave(void* userData, struct wl_surface* surface,
  ...
  for (i = 0, found = GLFW_FALSE; i < window->wl.monitorsCount - 1; ++i) {
    if (monitor == window->wl.monitors[i]) found = GLFW_TRUE;
...
static const struct wl_surface_listener surfaceListener = {surfaceHandleEnter,
                                                           surfaceHandleLeave};
...
static void acquireMonitor(_GLFWwindow* window) {
  ...
  if (window->wl.decorations.top.surface) destroyFallbackDecorations(window);
}
...
static void releaseMonitor(_GLFWwindow* window) {
  ...
  if (window->wl.xdg.decorationMode !=
...
static GLFWbool createShellObjects(_GLFWwindow* window) {
  window->wl.xdg.surface =
      xdg_wm_base_get_xdg_surface(_glfw.wl.wmBase, window->wl.surface);
  ...
  xdg_toplevel_add_listener(window->wl.xdg.toplevel, &xdgToplevelListener,
                            window);
...
static void incrementCursorImage(_GLFWwindow* window) {
  ...
  if (!window || window->wl.decorations.focus != mainWindow) return;
      ...
      if (read(_glfw.wl.timerfd, &repeats, sizeof(repeats)) == 8) {
        for (uint64_t i = 0; i < repeats; i++) {
          _glfwInputKey(_glfw.wl.keyboardFocus, _glfw.wl.keyboardLastKey,
                        _glfw.wl.keyboardLastScancode, GLFW_PRESS,
...
//
static char* readDataOfferAsString(struct wl_data_offer* offer,
                                   const char* mimeType) {
  int fds[2];
...
static _GLFWwindow* findWindowFromDecorationSurface(
    struct wl_surface* surface, _GLFWdecorationSideWayland* which) {
...
static void pointerHandleEnter(void* userData, struct wl_pointer* pointer,
...
static void pointerHandleLeave(void* userData, struct wl_pointer* pointer,
                               uint32_t serial, struct wl_surface* surface) {
...
static void pointerHandleMotion(void* userData, struct wl_pointer* pointer,
...
static void pointerHandleButton(void* userData, struct wl_pointer* pointer,
  ...
  if (button == BTN_LEFT) {
    ...
    if (edges != XDG_TOPLEVEL_RESIZE_EDGE_NONE) {
      xdg_toplevel_resize(window->wl.xdg.toplevel, _glfw.wl.seat, serial,
                          edges);
      return;
    }
  ...
  if (window->wl.decorations.focus != mainWindow) return;
...
static void pointerHandleAxis(void* userData, struct wl_pointer* pointer,
...
static void keyboardHandleKeymap(void* userData, struct wl_keyboard* keyboard,
  ...
  keymap = xkb_keymap_new_from_string(_glfw.wl.xkb.context, mapStr,
  ...
  composeTable = xkb_compose_table_new_from_locale(
      _glfw.wl.xkb.context, locale, XKB_COMPOSE_COMPILE_NO_FLAGS);
  if (composeTable) {
    composeState =
  ...
  _glfw.wl.xkb.controlIndex =
      xkb_keymap_mod_get_index(_glfw.wl.xkb.keymap, "Control");
  _glfw.wl.xkb.altIndex = xkb_keymap_mod_get_index(_glfw.wl.xkb.keymap, "Mod1");
  _glfw.wl.xkb.shiftIndex =
      xkb_keymap_mod_get_index(_glfw.wl.xkb.keymap, "Shift");
  _glfw.wl.xkb.superIndex =
      xkb_keymap_mod_get_index(_glfw.wl.xkb.keymap, "Mod4");
  _glfw.wl.xkb.capsLockIndex =
      xkb_keymap_mod_get_index(_glfw.wl.xkb.keymap, "Lock");
  _glfw.wl.xkb.numLockIndex =
      xkb_keymap_mod_get_index(_glfw.wl.xkb.keymap, "Mod2");
}

static void keyboardHandleEnter(void* userData, struct wl_keyboard* keyboard,
                                uint32_t serial, struct wl_surface* surface,
...
static void keyboardHandleLeave(void* userData, struct wl_keyboard* keyboard,
                                uint32_t serial, struct wl_surface* surface) {
...
static void keyboardHandleKey(void* userData, struct wl_keyboard* keyboard,
                              uint32_t serial, uint32_t time, uint32_t scancode,
                              uint32_t state) {
  _GLFWwindow* window = _glfw.wl.keyboardFocus;
  if (!window) return;
...
static void keyboardHandleModifiers(void* userData,
  ...
  xkb_state_update_mask(_glfw.wl.xkb.state, modsDepressed, modsLatched,
...
static void seatHandleName(void* userData, struct wl_seat* seat,
                           const char* name) {}
...
static void dataDeviceHandleDataOffer(void* userData,
  ...
  _GLFWofferWayland* offers = realloc(_glfw.wl.offers, _glfw.wl.offerCount + 1);
  if (!offers) {
    _glfwInputError(GLFW_OUT_OF_MEMORY, NULL);
    return;
  }
...
static void dataDeviceHandleEnter(void* userData, struct wl_data_device* device,
                                  uint32_t serial, struct wl_surface* surface,
  ...
  for (unsigned int i = 0; i < _glfw.wl.offerCount; i++) {
    if (_glfw.wl.offers[i].offer == offer) {
      ...
      if (surface) window = wl_surface_get_user_data(surface);
...
int _glfwPlatformCreateWindow(_GLFWwindow* window,
  ...
  if (ctxconfig->client != GLFW_NO_API) {
    if (ctxconfig->source == GLFW_EGL_CONTEXT_API ||
      ...
      window->wl.egl.window = wl_egl_window_create(
...
void _glfwPlatformSetWindowAspectRatio(_GLFWwindow* window, int numer,
  ...
  if (numer != GLFW_DONT_CARE && denom != GLFW_DONT_CARE) {
    const float aspectRatio =
        (float)window->wl.width / (float)window->wl.height;
...
void _glfwPlatformGetFramebufferSize(_GLFWwindow* window, int* width,
  ...
  if (width) *width *= window->wl.scale;
  if (height) *height *= window->wl.scale;
}
...
void _glfwPlatformGetWindowContentScale(_GLFWwindow* window, float* xscale,
                                        float* yscale) {
  if (xscale) *xscale = (float)window->wl.scale;
  if (yscale) *yscale = (float)window->wl.scale;
}
...
void _glfwPlatformSetWindowMonitor(_GLFWwindow* window, _GLFWmonitor* monitor,
  ...
  if (window->monitor == monitor) {
    if (!monitor) _glfwPlatformSetWindowSize(window, width, height);
...
float _glfwPlatformGetWindowOpacity(_GLFWwindow* window) { return 1.f; }
...
GLFWbool _glfwPlatformRawMouseMotionSupported(void) { return GLFW_TRUE; }
...
void _glfwPlatformWaitEvents(void) { handleEvents(NULL); }

void _glfwPlatformWaitEventsTimeout(double timeout) { handleEvents(&timeout); }
...
void _glfwPlatformGetCursorPos(_GLFWwindow* window, double* xpos,
                               double* ypos) {
...
void _glfwPlatformSetCursorPos(_GLFWwindow* window, double x, double y) {
  if (isPointerLocked(window)) {
    zwp_locked_pointer_v1_set_cursor_position_hint(
        window->wl.pointerLock.lockedPointer, wl_fixed_from_double(x),
...
const char* _glfwPlatformGetScancodeName(int scancode) {
  if (scancode < 0 || scancode > 255 ||
    ...
    _glfwInputError(GLFW_INVALID_VALUE, "Wayland: Invalid scancode %i",
                    scancode);
    return NULL;
  }
  ...
  xkb_keymap_key_get_syms_by_level(_glfw.wl.xkb.keymap, keycode, layout, 0,
                                   &keysyms);
  if (keysyms == NULL) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
...
int _glfwPlatformGetKeyScancode(int key) { return _glfw.wl.scancodes[key]; }
...
static void relativePointerHandleRelativeMotion(
    void* userData, struct zwp_relative_pointer_v1* pointer, uint32_t timeHi,
    uint32_t timeLo, wl_fixed_t dx, wl_fixed_t dy, wl_fixed_t dxUnaccel,
...
static void lockedPointerHandleLocked(
    void* userData, struct zwp_locked_pointer_v1* lockedPointer) {}
...
static void lockedPointerHandleUnlocked(
    void* userData, struct zwp_locked_pointer_v1* lockedPointer) {}
...
static void lockPointer(_GLFWwindow* window) {
  ...
  zwp_locked_pointer_v1_add_listener(lockedPointer, &lockedPointerListener,
                                     window);

  window->wl.pointerLock.relativePointer = relativePointer;
  window->wl.pointerLock.lockedPointer = lockedPointer;

  wl_pointer_set_cursor(_glfw.wl.pointer, _glfw.wl.pointerEnterSerial, NULL, 0,
                        0);
}
...
void _glfwPlatformSetCursor(_GLFWwindow* window, _GLFWcursor* cursor) {
  ...
  if (window != _glfw.wl.pointerFocus ||
      window->wl.decorations.focus != mainWindow)
    return;
  ...
  if (window->cursorMode == GLFW_CURSOR_NORMAL) {
    if (cursor)
      ...
      _GLFWcursorWayland cursorWayland = {
          defaultCursor, defaultCursorHiDPI, NULL, 0, 0, 0, 0, 0};
      setCursorImage(window, &cursorWayland);
    }
  } else if (window->cursorMode == GLFW_CURSOR_DISABLED) {
    if (!isPointerLocked(window)) lockPointer(window);
  } else if (window->cursorMode == GLFW_CURSOR_HIDDEN) {
    wl_pointer_set_cursor(_glfw.wl.pointer, _glfw.wl.pointerEnterSerial, NULL,
                          0, 0);
  }
}
...
void _glfwPlatformSetClipboardString(const char* string) {
  ...
  wl_data_source_add_listener(_glfw.wl.selectionSource, &dataSourceListener,
                              NULL);
  wl_data_source_offer(_glfw.wl.selectionSource, "text/plain;charset=utf-8");
  wl_data_device_set_selection(_glfw.wl.dataDevice, _glfw.wl.selectionSource,
                               _glfw.wl.serial);
}
...
const char* _glfwPlatformGetClipboardString(void) {
  ...
  free(_glfw.wl.clipboardString);
  _glfw.wl.clipboardString = readDataOfferAsString(_glfw.wl.selectionOffer,
...
int _glfwPlatformGetPhysicalDevicePresentationSupport(VkInstance instance,
  ...
  return vkGetPhysicalDeviceWaylandPresentationSupportKHR(device, queuefamily,
                                                          _glfw.wl.display);
}
